...
In *vpObject*, pass the 4D View Pro object that you want to convert. This object must have been previously parsed using [`VP Export to object`](cmd_vpExportToObject.md) or saved using [`VP EXPORT DOCUMENT`](cmd_vpExportDocument.md).  
...
>Gridline visibility depends on document attribute defined with [`VP SET PRINT INFO`](cmd_vpSetPrintInfo.md). 
